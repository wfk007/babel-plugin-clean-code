    ...
    visitor: {
      CallExpression: function CallExpression(path, _ref) {
        ...
        if (clearConsole && consoleLevel instanceof Array && consoleLevel.length > 0) {
          if (path.node.callee.object && path.node.callee.object.name === "console" && path.node.callee.property && consoleLevel.indexOf(path.node.callee.property.name) !== -1 && (path.parent.type === "ArrowFunctionExpression" || path.parent.type === "ReturnStatement")) {
            path.remove();
          }
        }
      },
